...
def init_warehouses_and_connect(worldSocket: socket, worldId: int):
    ...
    # print(received_proto_message(worldSocket, amazonBuffer.AResponses))
...
def socket_server():
    ...
    init_warehouses_and_connect(world_socket, int(response.decode()))

    world_thread = threading.Thread(
        target=handle_world_responses, args=(world_socket, ups_socket)
    )

    web_thread = threading.Thread(
        target=handle_user_requests, args=(world_socket, ups_socket, web_socket)
    )
